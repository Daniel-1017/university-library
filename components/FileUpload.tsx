...
const FileUpload = ({ type, accept, placeholder, folder, variant, onFileChange }: Props) => {
        ...
        if (type === "image") {
            if (file.size > 20 * 1024 * 1024) {
                ...
                toast({
                    ...
                    description: "Please upload a file tah is less than 20MB in size",
                    variant: "destructive",
                });
                return false;
            }
            ...
            if (file.size > 50 * 1024 * 1024) {
                ...
                toast({
                    title: "File size too large",
                    description: "Please upload a file tah is less than 50MB in size",
                    variant: "destructive",
                });
                return false;
            }
    ...
    return (
        <ImageKitProvider publicKey={config.env.imagekit.publicKey} authenticator={authenticator} urlEndpoint={config.env.imagekit.urlEndpoint}>
            <IKUpload
                className='hidden'
                ref={ikUploadRef}
                onError={onError}
            ...
            {progress > 0 && progress !== 100 && (
                <div className='2-full rounded-full bg-green-200'>
